...
class RuleEngine:
    def __init__(self, db_conn=None):
        ...
        # --- Parking Config ---
        self.no_parking_zones = []  # List of polygons [(x1,y1), (x2,y2), ...]
        self.stationary_threshold_px = 5  # Allowed "jitter" in pixels
    ...
    def check_illegal_parking(self, tracked_objects):
        parking_violations = []
        ...
        # Ensure coordinates are floats for the OpenCV test
            point = (float(centroid[0]), float(centroid[1]))
        
            is_in_zone = False
            for zone in self.no_parking_zones:
            # We use >= 0 because 0 is on the line and 1 is inside
                if cv2.pointPolygonTest(np.array(zone, np.int32), point, False) >= 0:
                    is_in_zone = True
                    break
        
            if is_in_zone:
                if tid not in self.stationary_history:
                    self.stationary_history[tid] = {"start_time": current_time, "last_pos": centroid}
                else:
                    last_pos = self.stationary_history[tid]["last_pos"]
                    dist = np.sqrt((centroid[0]-last_pos[0])**2 + (centroid[1]-last_pos[1])**2)
                
                # If the car moves more than 5 pixels, it's not "parked" - reset timer
                    if dist > self.stationary_threshold_px:
                        self.stationary_history[tid] = {"start_time": current_time, "last_pos": centroid}
                    else:
                        duration = current_time - self.stationary_history[tid]["start_time"]
                    # TRIGGER CONDITION
                    if duration > self.parking_violation_time_s:
                        parking_violations.append({
                            "type": "illegal_parking",
                            "track_id": tid,
                            "bbox": bbox,
                            "reason": f"Stationary for {int(duration)}s"
                        })
        else:
            if tid in self.stationary_history:
