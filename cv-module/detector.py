...
class TrafficDetector:
    def __init__(self,
        ...
        # Inside TrafficDetector.__init__
        self.rule_engine.no_parking_zones = [
            [
                (5, 980),   # Bottom Left (covers wide footpath/storefront)
                (750, 980),  # Bottom Right (middle of the road)
                (600, 200),  # Top Right (near the horizon)
                (100, 200)   # Top Left (near the top of the storefront)
            ]
        ]
        # ✅ Plate model load
    ...
    def detect_frame(self, frame):
        ...
        # 2. Draw the No-Parking Zones (ADD THIS HERE)
        for zone in self.rule_engine.no_parking_zones:
            if len(zone) > 0:
                pts = np.array(zone, np.int32).reshape((-1, 1, 2))
                cv2.polylines(annotated, [pts], True, (0, 0, 255), 2)
                # Use the first point of the polygon to place the label
                cv2.putText(annotated, "NO PARKING ZONE", (zone[0][0], max(20, zone[0][1]-10)), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)

        # 3. Draw the Allowed Direction Arrow (Existing)
        ax, ay = allowed_dir
